const consumoForm = document.querySelector('#consumoForm'),
      listConsumo = document.querySelector('#listConsumo tbody');
...
function editData(data) {
  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      table.ajax.reload();
      if (result.result === 'true') {
        $('#tblConsumo').load(document.URL + ' #fleetsTable');
        // mostrar notificación de Correcto
...
function mostrarNotificacion(mensaje, clase) {
  ...
  // formulario
  consumoForm.insertBefore(notificacion, document.querySelector('form legend'));
